...
class BrainVolumetrics():
    ...
    def compute_volumetrics(self):
        ...
        for f in self.filelist:
            ...
            if os.path.exists(icv_file):
                ...
                icv_vol = np.sum(icv)*0.8*0.8*0.8
                new_df['ICV'] = icv_vol
            else:
            ...
            if os.path.exists(lesion_file):
                ...
                lesion_vol = np.sum(lesion)*0.8*0.8*0.8
    ...
    def save_df(self, output_dir, output_prefix):
...
# path = "../RIS_ECTRIMS2021/*/*/*_macruise_volumes.csv"
# all_files = sorted(glob.glob(path))
#
# # Grab MACruise results from each scan
# all_df = []
# filenames = []
# for f in all_files:
#     df = pd.read_csv(f, sep=',',header=0,index_col=1)
...
#
# merged_df = pd.concat(all_df, sort=False)
# #merged_df.to_csv('/Users/jiwonoh/Desktop/LME_Scans/LME_macruise_vol.csv')
# cols = merged_df.columns.tolist()
#
# cols = merged_df.columns.tolist()
#
...
# merged_df.fillna(0)
#
# merged_df.to_csv('RIS_ectrims_macruise_vol_july12.csv')